...
def test_deepseek_login(context, base_url, app_credentials):
    # Verifies that user can log in with provided credentials and deepseek is connected.
    model = "deepseek"
...
def test_deepseek_chat(context, base_url, app_credentials):
    ...
    expect(chat_page.prompt_response_field, "Model is not responding after sending a prompt.").to_be_visible()
    expect(chat_page.prompt_response_field, "Model response is not correct").to_contain_text(expected_response)
