...
def test_azuracast_radio_station_creation(context, base_url, app_credentials):
    ...
    setup_page = AzuracastSetupPage(context)
    setup_page.create_station(STATION_NAME)
    ...
    expect(
        context.get_by_role("cell", name=STATION_NAME, exact=False),
        f"Station '{STATION_NAME}' was not found in the dashboard after wizard completion.",
    ).to_be_visible()
