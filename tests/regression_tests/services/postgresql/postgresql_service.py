...
class PostgresqlService:
    """
    Client actions for the PostgreSQL Marketplace app over SSH.
    """
    ...
    def query(self, password, sql, database=None, timeout=30):
        cmd = (
            f"PGPASSWORD={shlex.quote(password)} psql -h {self.HOST} "
            f"-U {self.DB_USER} -d {shlex.quote(database or self.DEFAULT_DB)} "
