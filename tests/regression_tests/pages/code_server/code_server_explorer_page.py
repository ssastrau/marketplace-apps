...
class CodeServerExplorerPage(BasePage):
    ...
    def open_folder(self, base_url: str, folder: str = "/home/admin"):
        """Navigate to code-server with the given folder open."""
        self.navigate(f"{base_url}/?folder={folder}")

    def handle_trust_dialog(self):
        """Click 'Yes, I trust the authors' if the workspace trust dialog appears."""
        try:
            self.trust_yes_button.click(timeout=10000)
        except Exception:
            pass  # Dialog not shown — folder already trusted from a previous test

    def wait_for_explorer(self):
        """Wait until the Explorer section is visible and ready."""
        self.explorer_section.wait_for(state="visible", timeout=15000)

    def create_new_file(self, filename: str):
        """Hover the Explorer section to reveal the toolbar, then create a new file."""
    ...
    def get_file_item(self, filename: str):
        """Return a locator for a file entry in the Explorer tree."""
