...
def ssh_connection(host: str, username: str, password: str, timeout: int = 30):
    ...
    This context manager handles the setup and teardown of a paramiko SSHClient.
    It automatically adds missing host keys and guarantees that the connection
    is closed when the block exits, even if an exception is raised.
...
def get_credentials_via_ssh(host: str, username: str, password: str, remote_path: str) -> dict:
    ...
    Connects to the specified host, reads the file at `remote_path`, and
    parses it into a key-value dictionary. The remote file is expected to
    have lines in a `Key: Value` format. Empty lines and lines starting
    with `#` are ignored.
