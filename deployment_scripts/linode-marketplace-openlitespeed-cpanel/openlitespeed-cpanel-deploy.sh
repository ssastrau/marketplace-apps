...
# moved to root dir because cpanel install will remove anything in tmp (matches
# linode-marketplace-cpanel-almalinux's deployment_scripts/cpanel-almalinux-deploy.sh)
...
function run {
	# install dependencies (AlmaLinux 10 only - see architecture_decisions.md D3)
	dnf update -y
...
function final_run {
	...
	# run playbook (no provision.yml - this app has no generated sudo user/DNS, see D1)
	ansible-playbook -v site.yml
}

function installation_complete {
	echo "Installation complete. WHM: https://$(hostname -I | awk '{print $1}'):2087 (root SSH password). LiteSpeed WebAdmin credentials: /root/.credentials"
}
