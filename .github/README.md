...
**Trigger:** Pull requests to any branch, or manual dispatch (`workflow_dispatch`).

**Purpose:** Validates and deploys only the apps that were **changed** in the pull request or branch.

**App detection:** Compares changed files in `apps/` and `deployment_scripts/` against `main`. Only apps with a `deployment_scripts/<app>/linode-config.sh` are deployed.

---

### 2. Scheduled Workflow (`scheduled.yml`)

**Trigger:** Weekly cron schedule — every **Sunday at 18:00 UTC**.

**Purpose:** Runs the full pipeline against **all apps** on the `main` branch to catch regressions.
