...
Write one file per generation session to
`.claude/commands/ui-regression-tests/logs/{YYYY-MM-DD}-{app}.md` — date first (get it with
`date '+%Y-%m-%d'`), then the app name, e.g. `2026-06-17-apache-airflow.md`. Create the `logs/`
directory if it doesn't exist. If a file already exists for that date and app, append a new section
rather than overwriting.

**No sensitive data.** Never write credentials, passwords, tokens, or credential-file contents into
the log. Credential *key names* are fine; their *values* are not. **Do not include the base URL.**

````markdown
# {App} — generation session log ({YYYY-MM-DD})
