...
NEW APP:   /newapp-start   <app> --repo <git-url>                  ───┤   each →
            ...
            /ui-regression-tests <app>     ──STOP: full UI suite green on a fresh redeploy──▶
...
| 6 UI tests | `/ui-regression-tests` | `ui_testing.md` + `tests/regression_tests/` suite files | full suite green on a fresh redeploy |
...
`validation_findings.md`, `ui_testing.md`, `pr_description.md` — see the table's "Owns artifact"
column). When a
