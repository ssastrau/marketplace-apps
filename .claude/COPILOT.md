...
This pipeline was built as Claude Code skills, with the regression-test skills also exposed as
GitHub Copilot project skills. The regression skills live in `.github/skills/` because this
Copilot setup does not discover skills from `.claude/skills/` reliably; the remaining pipeline
...
The regression skills therefore live in `.github/skills/<name>/SKILL.md`, where this setup can
load them reliably. The Claude-only frontmatter keys (`disable-model-invocation`, `arguments`) are
simply ignored by Copilot; the skill **name** is inferred from the directory. Verify with:

```bash
copilot skill list        # from inside marketplace-apps
```

You should see the Copilot-exposed regression phases: `ui-regression-tests` and
